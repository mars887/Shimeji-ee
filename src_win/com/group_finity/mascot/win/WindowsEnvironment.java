...
import com.group_finity.mascot.environment.GetBorderPlus;
...
class WindowsEnvironment extends Environment {
    ...
    public static final ArrayList<WindowHolder> activesIE = new ArrayList<>();

    private static Pointer activeIEobject = null;
    private static boolean activeIEObjectHasTick = false;

    private static String[] windowTitles = null;

    public static boolean isBlackList;

    public WindowsEnvironment(boolean property) {
    ...
    private static boolean isIE(final Pointer ie) {
        ...
        for (String windowTitle : windowTitles) {
            ...
            if (isBlackList) {
                bll = !ieTitle.contains(windowTitle);
            } else {
                bll = ieTitle.contains(windowTitle);
            }
            if (!windowTitle.trim().isEmpty() && bll) {
    ...
    private static IEResult isViableIE(Pointer ie) {
        if (User32.INSTANCE.IsWindowVisible(ie) != 0) {
            ...
            if (result.longValue() != 0x80070057L && (result.longValue() != 0 || flagsRef.getValue() != 0)) // unsupported on 7 so skip the check
...
//                return IEResult.INVALID;
    ...
    private static void findActiveIE() {
        if (activeIEObjectHasTick) return;
        activeIEobject = null;

        ArrayList<WindowHolder> newl = new ArrayList<>();

        User32.INSTANCE.EnumWindows((ie, data) -> {

            switch (isViableIE(ie)) {
                case IE:
                    if (activeIEobject == null) activeIEobject = ie;

                    Area ar = new Area();
                    ar.set(getIERect(ie));

                    newl.add(new WindowHolder(ar, ie));
        ...
        activeIEObjectHasTick = true;

        WindowsMapper.Companion.applyTo(activesIE, newl);
...
//        return null;
    ...
    private static Rectangle getIERect(Pointer ie) {
        ...
        int rectX = Math.max(0, out.left + in.left);
        int rectY = Math.max(0, out.top + in.top);
        int xOffset = (out.left + in.left) - rectX;
        int yOffset = (out.top + in.top) - rectY;

        return new Rectangle(rectX, rectY, in.Width() + xOffset, in.Height() + yOffset);
    }
    ...
    public void tick() {
        ...
        activeIEObjectHasTick = false;
        findActiveIE();

        GetBorderPlus.Companion.windowsEnviromentTick(this);
        final Rectangle ieRect = getIERect(activeIEobject);
    ...
    public void moveActiveIE(final Point point) {
        moveIE(activeIEobject, new Rectangle(point.x, point.y, activeIE.getWidth(), activeIE.getHeight()));
    }
    ...
    @Override
    public ArrayList<WindowHolder> getActivesIE() {
        return activesIE;
    }

    @Override
    public String getActiveIETitle() {
        return getTitleOf(activeIEobject);
    }

    public static String getTitleOf(Pointer ie) {
        final char[] title = new char[1024];

...
public class ImagePairLoader {
    ...
    public static void load(final String name, final String rightName, final Point center, final double scaling, final Filter filter) throws IOException {
        ...
        final BufferedImage leftImage = scale(premultiply(ImageIO.read(Files.newInputStream(Paths.get("./img/" + name)))), scaling, filter);
        final BufferedImage rightImage;
        if (rightName == null)
            rightImage = flip(leftImage);
        else
            rightImage = scale(premultiply(ImageIO.read(Files.newInputStream(Paths.get("./img/" + rightName)))), scaling, filter);

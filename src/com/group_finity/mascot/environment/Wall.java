...
public class Wall implements Border {
	...
	public boolean isLefterThan(final Point location) {
		return getArea().isVisible() && (getX() < location.x) && (area.getTop() <= location.y)
				&& (area.getBottom() >= location.y);
	}

	public boolean isRighterThan(final Point location) {
		return getArea().isVisible() && (getX() > location.x) && (area.getTop() <= location.y)
				&& (area.getBottom() >= location.y);
	}

...
public class FloorCeiling implements Border {
    ...
    public int isOnLine(int x, int yfrom, int yto) {
        if (getArea().isVisible() && (getY() >= yfrom && getY() <= yto) && (getLeft() <= x)
                && (x <= getRight())) {
            return yfrom - (yfrom - getY());
        } else return Integer.MIN_VALUE;
    }


    public boolean isHigherThan(final Point location) {
        return getArea().isVisible() && (getY() > location.y) && (getLeft() <= location.x)
                && (location.x <= getRight());
    }


    public boolean isLowerThan(final Point location) {
        return getArea().isVisible() && (getY() < location.y) && (getLeft() <= location.x)
                && (location.x <= getRight());
    }
